...
class data_class():
	...
	def J (self, x, dt, x_data):
		result = 0
		result = np.sum(dt * np.abs(x - x_data)**2, axis = )
		for i in range(len(x)):
			result += dt * np.linalg.norm(x[i] - x_data[i])**2 # .abs -> .linalg.norm
		return result
...
def p1(dt, data:data_class, v_max, d):
	...
	for i, j in enumerate(t_arr):
		if i == 0:
			...
			p = p + dt * 2*np.abs(data.get_numx()[0,-i-1]-data.get_x()[0,-i-1]) - dt * p * (v_max/d[-i-1]) # [-i-1] - "-1" ist dafür, damit tatsächlich bei bspw. i = 1, -> t = T - dt, das vorletzte element ausgewählt wird (index [-2])
		p1[-i-1] = p
	return p1
...
def p2_N_1(dt, data:data_class, pi_1, d, n, v_max): # n tes auto
	...
	for i, j in enumerate(t_arr):
		if i == 0:
			...
			p = p + dt * 2*np.abs(data.get_numx()[n,-i-1]-data.get_x()[n,-i-1]) - dt * p * (v_max/d[-i-1]) + dt * pi_1[-i-1] * v_max/d[-i-1]
		pi[-i-1] = p
	return pi
...
def pN(dt, data:data_class, pi_1, d, v_max): # n tes auto
	...
	for i, j in enumerate(t_arr):
		if i == 0:
			...
			p = p + dt * 2*np.abs(data.get_numx()[-1,-i-1]-data.get_x()[-1,-i-1]) + dt * pi_1[-i-1] * v_max/d[-i-1]
		pi[-i-1] = p
	return pi
...
def gradient(P_ges, d, v_max, L, dt):
    skvmax = integral(P_ges[-1,:] + np.sum(np.array([P_ges[i,:] * np.log(d[i,:]/L) for i in range(P_ges.shape[0]-1)]), axis=0), dt)
    skL = integral(np.sum(np.array([P_ges[i,:] * v_max/L for i in range(P_ges.shape[0]-1)]), axis=0), dt)
    return (skvmax, skL)

# f funktion, min, max weglassen, und update für xn auf vmax setzen